...
const Sidebar: React.FC<SidebarProps> = ({ variant = 'dashboard' }) => {
    ...
    const dashboardLinks: SidebarLink[] = [
        ...
        { to: '/admin/relatorios', icon: 'bar_chart', label: 'Relatórios Estratégicos' },
    ];
    ...
    return (
        <aside className={`w-72 flex-shrink-0 flex flex-col justify-between py-8 ${variant === 'dashboard' ? 'glass-sidebar' : 'glass-panel h-screen sticky top-0 hidden lg:flex rounded-r-[3rem] my-0 shadow-2xl'}`}>
